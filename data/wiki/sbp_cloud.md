---
doc_id: "SBP_Cloud"
name: "SBP Framework on Outsourcing to CSPs"
regulator: "SBP"
reference: "BPRD Circular 01 of 2023 — Framework on Outsourcing to Cloud Service Providers"
date: "2023"
applicable_to: "Banks, DFIs, MFBs, Digital Banks, EMIs (REs)"
source_url: "https://www.sbp.org.pk/bprd/2023/C1-Annix-A.pdf"
tags: [sbp, pakistan-regulation, biometric, cloud, collection, outsourcing, recovery, tier]
status: active
---


# SBP Framework on Outsourcing to CSPs

**Regulator:** SBP  
**Reference:** BPRD Circular 01 of 2023 — Framework on Outsourcing to Cloud Service Providers  
**Date:** 2023  
**Applicable to:** Banks, DFIs, MFBs, Digital Banks, EMIs (REs)  
**Source:** [https://www.sbp.org.pk/bprd/2023/C1-Annix-A.pdf](https://www.sbp.org.pk/bprd/2023/C1-Annix-A.pdf)

## Purpose & Scope

Specialised overlay on the outsourcing framework permitting REs to use public/private/hybrid cloud, subject to risk categorisation, due diligence, data localisation and SBP approval.

## Key Sections / Requirements

- Scope — material vs non-material workloads on cloud
- Data classification and localisation requirements (sensitive data must remain onshore)
- CSP due diligence (certifications: ISO 27001, SOC 2, PCI-DSS where relevant)
- Mandatory contract clauses — SBP audit rights, data residency, breach notification
- Encryption (in-transit and at-rest), key management
- Concentration risk — limits on dependence on single CSP/region
- Exit strategy and reversibility plan
- Incident reporting and BCP/DR
- SBP may instruct shifting workloads to onshore CSPs
- Continuous monitoring and periodic risk reassessment

## Auto-Extracted Section Headings (from source)

- 1. This framework sets out minimum requirements for REs to outsource their material and
- 2. This framework shall:
- 1. For the purpose of these regulations, material workload means all systems, applications,
- 2. REs may outsource their workloads to CSPs in the following manner:
- 3. Outsourcing of services to CSPs does not absolve the REs from their prime responsibilities
- 4. SBP may instruct any RE to restrict outsourcing of their workloads to CSPs due to its
- 6. SBP may instruct REs to shift their cloud based workloads to SBP designated onshore
- 2. Conduct appropriate due diligence of CSPs and proactively identify any risks emanating
- 3. Update their Enterprise Risk Management framework or other relevant policies for
- 4. Delegate cloud specific governance responsibilities such as for overseeing adherence to
- 5. Undertake all CO arrangements through legally binding SLAs, which shall at least include
- 6. Define and agree upon the roles and responsibilities of the IT and operational
- 7. Ensure that the CO arrangements are compliant with the relevant legal and regulatory
- 1. Evaluation of feasibility of CO arrangements including cost effectiveness, quality of
- 5. Assessment of CSPs ability to comply with necessary minimum controls including

## Detected Keywords

biometric, cloud, collection, outsourcing, recovery, tier

<!-- KG-LINKS -->

## Related concepts

- [[concepts/bcp-dr|Business Continuity & Disaster Recovery]]
- [[concepts/biometric|Biometric Verification]]
- [[concepts/cdd|Customer Due Diligence (CDD)]]
- [[concepts/classification-provisioning|Loan Classification & Provisioning]]
- [[concepts/cloud|Cloud / CSP Outsourcing]]
- [[concepts/data-localization|Data Localization / Residency]]
- [[concepts/outsourcing|Outsourcing]]

## Related entities

- [[entities/bank|Bank]]
- [[entities/dfi|Development Finance Institution (DFI)]]
- [[entities/digital-bank|Digital Bank (DRB / DFB)]]
- [[entities/emi|Electronic Money Institution (EMI)]]
- [[entities/mfb|Microfinance Bank (MFB)]]
- [[entities/reporting-entity|Reporting Entity (RE)]]
- [[entities/sbp|State Bank of Pakistan (SBP)]]
